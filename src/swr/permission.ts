...
export const usePermissionApproveList = (pagination: Pagination, filter: Filter) => {
  ...
  const getType = filter.type !== "" ? `&type=${filter.type}` : "";

  const getQuery = `${getPage}${getLimit}${getSearch}${getStatus}${getStartDate}${getEndDate}${getType}`;
  const getKeys = filter.isSet ? `/api/permission/list_approve?${getQuery}` : null;
