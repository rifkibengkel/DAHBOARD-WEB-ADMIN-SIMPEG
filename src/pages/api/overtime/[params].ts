  ...
  try {
    ...
    if (req.method === "GET") {
      if (req.query.params === "detail") {
        ...
        const response = await api.get(`/overtime/detail/${getId}`, { headers });
      ...
      if (req.query.params === "list_approve") {
        ...
        const response = await api.get(`/overtime/approve?${getQuery}`, { headers });
        return res.status(response.status).json(response.data);
      }
    }
    if (req.method === "POST") {
      if (req.query.params === "approved") {
        const response = await api.post(`/overtime/approve`, req.body, { headers });
        return res.status(response.status).json(response.data);
      }
    }
