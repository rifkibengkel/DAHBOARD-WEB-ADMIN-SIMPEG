...
import { DetailPermission, ModalVerifPermissionProps } from "../VerifPermission.types";
import { VerifPermissionSchema } from "../VerifPermission.schema";

const ModalVerifPermission: React.FC<ModalVerifPermissionProps> = ({ keys }) => {
  const { states, actions } = useUtils();
  const { mutate } = useSWRConfig();
  const { modal_detail } = states;

  const [permission, setPermission] = React.useState<DetailPermission | null>(null);
  ...
  const formik = useFormik({
    ...
    validationSchema: VerifPermissionSchema,
    ...
    const fetchNews = async () => {
      if (modal_detail.id) {
        ...
        if (response.status === 200) {
          setPermission(getResponse.data);
        }
    ...
    >
      ...
      <FormikProvider value={formik}>
                ...
                >
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      {permission ? (
                        <Typography fontSize="12px" color={theme.palette.text.secondary}>
                          {dayjs(permission.startDate).format("DD MMM YYYY")} {"->"} {dayjs(permission.endDate).format("DD MMM YYYY")}
                        </Typography>
                      ) : (
                        <Skeleton height={15} width={100} />
                      )}
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      {permission ? (
                        <Typography fontSize="12px" color={theme.palette.text.secondary}>
                          {permission.approvals.find((approval) => approval.position === "ASISTEN MANAGER")?.statusText || "-"}
                        </Typography>
                      ) : (
                        <Skeleton height={15} width={100} />
                      )}
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      {permission ? (
                        <Typography fontSize="12px" color={theme.palette.text.secondary}>
                          {permission.approvals.find((approval) => approval.position === "MANAGER")?.statusText || "-"}
                        </Typography>
                      ) : (
                        <Skeleton height={15} width={100} />
                      )}
              ...
              <Grid item xs={6}>
                <FormControl fullWidth>
                  ...
                  <Field
                    ...
                    disabled={+formik.values.status === 1 || Number(permission?.status) === 2}
                  />
                </FormControl>
              </Grid>
            </Grid>
        ...
        <DialogActions sx={{ py: 2.5, px: 3 }}>
          ...
          <ButtonPrimary onClick={handleSubmit} disabled={formik.isSubmitting || permission?.status === 2} fullWidth>
            Submit
          </ButtonPrimary>
        </DialogActions>
      </FormikProvider>
...
export default ModalVerifPermission;
